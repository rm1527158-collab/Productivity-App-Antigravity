...
const CategoryDonut = () => {
  ...
  useAutoRefresh(fetchCategoryData);
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-popover p-3 border border-border rounded-xl shadow-lg shadow-primary/5">
          <p className="text-secondary-foreground font-bold font-serif mb-1">{payload[0].name}</p>
          <p className="text-sm text-muted-foreground">
            <span className="font-bold text-primary">{payload[0].value}</span> tasks
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="bg-card p-6 lg:p-8 rounded-3xl border border-border shadow-sm h-full hover:shadow-md transition-shadow duration-300 flex flex-col justify-between">
       <div className="mb-6">
            <h3 className="text-xl font-bold text-foreground">Task Distribution</h3>
      ...
      <div className="flex-1 w-full min-h-[200px] flex flex-col lg:flex-row items-center gap-6 lg:gap-8">
        ...
        <div className="w-full lg:w-1/2 h-[220px] lg:h-full relative flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-3xl lg:text-4xl font-bold text-foreground font-serif">{total}</span>
                <span className="text-[10px] lg:text-xs text-muted-foreground font-medium uppercase tracking-wider mt-1">Total</span>
            </div>
        </div>
            ...
            {data.map((item, index) => (
                <div key={index} className="flex items-center justify-between group cursor-default">
                    ...
                    <div className="flex items-center gap-2">
                         ...
                         <span className="text-xs text-muted-foreground bg-muted/30 px-1.5 py-0.5 rounded-md min-w-[32px] text-center font-mono">
